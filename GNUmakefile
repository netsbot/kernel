...
run-x86_64: ovmf/ovmf-code-$(KARCH).fd ovmf/ovmf-vars-$(KARCH).fd $(IMAGE_NAME).iso
	qemu-system-$(KARCH) \
		-M q35 \
		-drive if=pflash,unit=0,format=raw,file=ovmf/edk2-ovmf/ovmf-code-$(KARCH).fd,readonly=on \
		-drive if=pflash,unit=1,format=raw,file=ovmf/edk2-ovmf/ovmf-vars-$(KARCH).fd \
...
ovmf/ovmf-code-$(KARCH).fd: ovmf/edk2-ovmf.tar.gz
	tar -xf $< --strip-components=1 -C ovmf/ edk2-ovmf/ovmf-code-$(KARCH).fd
	# Architecture-specific padding logic remains
	case "$(KARCH)" in \
...
ovmf/ovmf-vars-$(KARCH).fd: ovmf/edk2-ovmf.tar.gz
	tar -xf $< --strip-components=1 -C ovmf/ edk2-ovmf/ovmf-vars-$(KARCH).fd
	case "$(KARCH)" in \
	   aarch64) dd if=/dev/zero of=$@ bs=1 count=0 seek=67108864 2>/dev/null;; \
	   loongarch64) dd if=/dev/zero of=$@ bs=1 count=0 seek=5242880 2>/dev/null;; \
	   riscv64) dd if=/dev/zero of=$@ bs=1 count=0 seek=33554432 2>/dev/null;; \
...
# New target to handle the download once
